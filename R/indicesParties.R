...
# Need to add a "typical party L plot"
#
#
# Currently:
...
rcparty$govdeclared <- car::recode(rcparty$rc.gov,"c('Sim','Nãoo','Obstrução')=TRUE;else=FALSE")
...
for(i in 1:nrow(party.data)){
    ...
    typical.pty$rc2 <- car::recode(typical.pty$rc,c("c('Com Governo','Contra Governo')='Votando';else='N�o Votando'"))  
